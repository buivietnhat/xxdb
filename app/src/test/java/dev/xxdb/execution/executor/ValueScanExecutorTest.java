...
class ValueScanExecutorTest {
  ...
  class InitTest {
    ...
    void columnsNotCorrect() {
      ValueScanPlan plan =
          new ValueScanPlan(
              "FOO",
              List.of("col2", "col1"),
              List.of(List.of(new StringValue("hello"), new IntValue(2))));
      ...
      List<Column> columns =
          List.of(
              new Column("col1", TypeId.INTEGER, 0, 0),
              new Column("col2", TypeId.VARCHAR, 100, 100));
    ...
    void columnsAndTypesCorrect() {
      ...
      List<Column> columns =
          List.of(
              new Column("FOO.col1", TypeId.VARCHAR, 0, 0),
  ...
  void typesIncorrect() {
    ...
    List<Column> columns =
        List.of(
            new Column("col1", TypeId.INTEGER, 0, 0), new Column("col2", TypeId.INTEGER, 100, 100));
    when(mockSchema.getColumns()).thenReturn(columns);
  ...
  class NextTest {
    // Testing strategy
    // + partition on ValueScanPlan:
    //    + columns are supplied fully as in the table schema, columns are supplied partially (the
    // rest must be NULL)
    //    + values len: 1, >1
    ...
    void fullColumnsWithSingleValue() throws ExecutionException {
      ...
      ValueScanPlan plan =
          new ValueScanPlan("FOO", List.of("col1", "col2"), List.of(List.of(hello, two)));
      ExecutionContext mockCtx = mock(ExecutionContext.class);
      Catalog mockCatalog = mock(Catalog.class);
      when(mockCtx.catalog()).thenReturn(mockCatalog);
      Schema mockSchema = mock(Schema.class);
    ...
    void fullColumnsWithMultipleValues() throws ExecutionException {
      ...
      ValueScanPlan plan =
          new ValueScanPlan(
              "EMPLOYEES",
              List.of("name", "salary"),
              List.of(List.of(john, salary1), List.of(jane, salary2)));
      ...
      List<Column> columns =
          List.of(
              new Column("EMPLOYEES.name", TypeId.VARCHAR, 0, 0),
