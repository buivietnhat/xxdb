...
public class Schema {
  ...
  public static class Builder {
    ...
    public Builder addVarcharColumn(String columnName) {
      schema.addColumn(
          new Column(columnName, TypeId.VARCHAR, currentColumnOffset, Value.VARCHAR_MAX_SIZE));
  ...
  public Schema join(final Schema other) {
    List<Column> clone = new ArrayList<>(columns.stream().toList());
  ...
  public Schema filter(List<String> keptCols) {
    ...
    keptCols.stream()
        .map(col -> columns.stream().filter(column -> column.name().equals(col)).findFirst().get())
  ...
  public boolean containsColumn(String column) {
    return columns.stream().anyMatch(col -> col.name().equals(column));
  }
