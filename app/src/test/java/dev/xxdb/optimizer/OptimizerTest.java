...
class OptimizerTest {
  ...
  class PredicateBuilderTest {
    ...
    void simplePredicate() {
      ...
      assertEquals(
          "SimplePredicate{table='FOO', column='FOO.col1', value=IntValue[value=20], op=EQUALS}",
          build.toString());
    }
    ...
    void andPredicate() {
      ...
      assertEquals(
          "AndPredicate{left=SimplePredicate{table='FOO', column='FOO.col1', value=IntValue[value=20], op=EQUALS}, "
  ...
  class RunTest {
    ...
    void insertTest() {
      ...
      assertEquals(
          "InsertPlan{tableName='Table', "
              ...
              + "[IntValue[value=3], StringValue[value=c]]]}}",
    ...
    void createTableTest() {
      ...
      assertEquals(
          "CreateTablePlan{tableName='Foo', columns=[Foo.col1, Foo.col2], types=[INT, VARCHAR]}",
          physicalPlan.toString());
    }
    ...
    void selectTestOnlyProjection() {
      ...
      Schema schema =
          schemaBuilder
              ...
              .addIntColumn("FOO.col3")
              .build();
      when(mockCatalog.getTableSchema(eq("FOO"))).thenReturn(Optional.of(schema));
      PhysicalPlan physicalPlan = optimizer.run(logicalPlan);
      assertEquals(
          "ProjectionPlan{columns=[FOO.col1, FOO.col2, FOO.col3], child=SequentialScanPlan{table='FOO'}}",
          physicalPlan.toString());
      assertEquals(
          "Schema{columns=[Column[name=FOO.col1, typeId=INTEGER, tupleOffset=0, length=4], "
    ...
    void selectTestProjectionAndPredicate() {
      ...
      PhysicalPlan physicalPlan = optimizer.run(logicalPlan);
      assertEquals(
          "ProjectionPlan{columns=[FOO.col1, FOO.col2, FOO.col3], "
              ...
              + "child=SequentialScanPlan{table='FOO'}}}",
          physicalPlan.toString());
    ...
    void selectTestProjectionAndPredicateAndJoinAndLimit() {
      String query =
          "SELECT FOO.col1, FOO.col2, BAR.col1, BAR.col2 FROM FOO JOIN BAR ON FOO.id = BAR.id "
      ...
      assertEquals(
          "LimitPlan{number=10, "
              ...
              + "rightChild='SequentialScanPlan{table='BAR'}'}}}}",
          physicalPlan.toString());
